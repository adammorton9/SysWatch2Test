...
namespace SysWatchTester
{
    public static class WebServiceClient
    {
        ...
        public static void SetTotalRequests(int requests)
        {
        ...
        public static void SendRequest(Request request)
        {
            switch (request.Environment)
            {
                case Environment.Dev:
                    ...
                    devSysWatchService.SubmitJob("0", 0, "0", request.Url, request.QueryString, 0, "test", "1");
                    break;
                case Environment.QA:
                    var qaSysWatchService = new QASysWatch2.XmlService();
                    qaSysWatchService.SubmitJob("0", 0, "0", request.Url, request.QueryString, 0, "test", "1");
                    break;
                case Environment.Test:
                    var testSysWatchService = new TestSysWatch2.XmlService();
                    testSysWatchService.SubmitJob("0", 0, "0", request.Url, request.QueryString, 0, "test", "1");
                    break;
                case Environment.UAT:
                    var uatSysWatchService = new UATSysWatch2.XmlService();
                    uatSysWatchService.SubmitJob("0", 0, "0", request.Url, request.QueryString, 0, "test", "1");
                    break;
                case Environment.Prod:
                    var prodSysWatchService = new ProdSysWatch2.XmlService();
                    prodSysWatchService.SubmitJob("0", 0, "0", request.Url, request.QueryString, 0, "test", "1");
                    break;
                default:
